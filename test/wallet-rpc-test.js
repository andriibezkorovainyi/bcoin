...
describe('Wallet RPC Methods', function() {
  ...
  for (const usePlugin of [true, false]) {
    describe(usePlugin ? 'Wallet as plugin' : 'Wallet as separate node', function () {
      let node, wdb, walletNode;
      if (usePlugin) {
        node = new FullNode({
        ...
        const plugin = node.require('walletdb');
        wdb = plugin.wdb;
      } else {
        node = new FullNode({
          network: network.type,
      ...
      it('should rpc listsinceblock', async () => {
        ...
        await assert.rejects(
          wclient.execute('listsinceblock', [nonexistentBlock]),
      ...
      it('should rpc abortRescan', async () => {
        ...
        // Rescan was aborted, wdb is stuck at height
