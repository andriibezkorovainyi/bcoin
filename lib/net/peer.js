...
class Peer extends EventEmitter {
    ...
    if (this.outbound) {
      ...
      if(this.options.neutrino) {
        if(!(this.services & services.NODE_COMPACT_FILTERS)){
          throw new Error('Peer does not support Neutrino.');
        }
      }
  ...
  sendCFilter(filterType, blockHash, filter) {
    const packet = new packets.CFilterPacket(
      filterType,
      blockHash,
      filter);
  ...
  sendCFHeaders(filterType, stopHash, previousFilterHeader, filterHashes) {
    ...
    this.logger.debug(
      ...
      filterType, stopHash, previousFilterHeader);
  ...
  sendCFCheckpt(filterType, stopHash, filterHeaders) {
    ...
    this.logger.debug(
      ...
      filterType, stopHash, filterHeaders.length);
