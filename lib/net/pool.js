...
class Pool extends EventEmitter {
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.CFCHECKPT:
        await this.handleCFCheckpt(peer, packet);
        break;
      case packetTypes.CFHEADERS:
        await this.handleCFHeaders(peer, packet);
        break;
      ...
      case packetTypes.CFILTER:
        await this.handleCFilters(peer, packet);
        break;
  ...
  /**
   * Handle peer `CFCheckpt` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {CFCheckptPacket} packet
   */

  async handleCFCheckpt(peer, packet) {
    if (!this.options.neutrino) {
      peer.ban();
      peer.destroy();
    }
  ...
  /**
   * Handle peer `CFHeaders` packet.
   * @method
   * @private
   * @param {Peer} peer - Sender.
   * @param {CFHeadersPacket} packet - Packet to handle.
   * @returns {void}
   */
  async handleCFHeaders(peer, packet) {
    this.logger.info('Received CFHeaders packet from %s', peer.hostname());
    if (!this.options.neutrino) {
      peer.ban();
      peer.destroy();
      return;
  ...
  async handleCFilters(peer, packet) {
    this.logger.info('Received CFilter packet from %s', peer.hostname());
    if (!this.options.neutrino) {
      peer.ban();
      peer.destroy();
      return;
    ...
    const blockHash = packet.blockHash;
    const filterType = packet.filterType;
    ...
    this.emit('cfilter', blockHeight, gcsFilter);
  }
