...
const digests = common.digests;
const {encoding} = bio;
const {inspectSymbol, taggedHash, util} = require('../utils');
const {strcmp} = util;
...
class Script {
  ...
  constructor(options) {
    ...
    // aka "validation_weight_left" in Bitcoin Core.
    // BIP342: "The sigops in tapscripts do not count towards
    // the block-wide limit of 80000 (weighted). Instead, there is a per-script
    // sigops budget. The budget equals 50 + the total serialized size in bytes
    // of the transaction input's witness (including the CompactSize prefix).
    // Executing a signature opcode (OP_CHECKSIG, OP_CHECKSIGVERIFY, or
    // OP_CHECKSIGADD) with a non-empty signature decrements the budget by 50.
    // If that brings the budget below zero, the script fails immediately.
    // Signature opcodes with unknown public key type and non-empty signature
    // are also counted"
    this._sigopBudget = null;
   ...
   * @param {Number?} digest - Signature hash digest.
   * @param {Coin[]} coins - Array of coins to spend (required for Tapscript).
   * @throws {ScriptError} Will be thrown on VERIFY failures.
   */

  execute(stack, flags, tx, index, value, digest, coins) {
    if (flags == null)
      flags = Script.flags.STANDARD_VERIFY_FLAGS;

    if (digest == null)
      digest = digests.LEGACY;

    // Taproot has no script execution. Raw public key is in the output,
    // signature is on top of the witness, both checked in verifyProgram().
    assert(digest !== digests.TAPROOT);

    if (!this._sigopBudget && digest === digests.TAPSCRIPT)
      throw new Error('Tapscript execution requires sigopBudget');

    // BIP342: "The maximum script size of 10000 bytes does not apply.
    // Their size is only implicitly bounded by the block weight limit...
    // The existing limit of 1000 elements in the stack and altstack
    // together after every executed opcode remains.
    // It is extended to also apply to the size of initial stack"
    if (digest === digests.TAPSCRIPT) {
      if (stack.length > consensus.MAX_SCRIPT_STACK)
    ...
    // BIP342: default position if no OP_CODESEPARATOR is executed
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      if (op.value === opcodes.OP_CODESEPARATOR && digest === digests.LEGACY &&
      ...
      switch (op.value) {
        ...
        case opcodes.OP_NOTIF: {
          ...
          if (!negate) {
            ...
            // BIP342: "The MINIMALIF rules, which are only a standardness rule
            // in P2WSH, are consensus enforced in Tapscript.""
            if (digest === digests.TAPSCRIPT
                || (digest === digests.WITNESSV0
        ...
        case opcodes.OP_CHECKSIGVERIFY: {
          ...
          if (digest !== digests.TAPSCRIPT) {
            ...
            // Note that sighash type is not derived yet.
            // We must check other properties of key and sig for soft fork
            // upgrades first, otherwise an unexpected sig format could throw.
            const sighashData = {
              index,
              value,
              digest,
            ...
            res = this.evalChecksigTapscript(
              tx,
              key,
              sig,
              sighashData,
        ...
        case opcodes.OP_CHECKSIGADD: {
          ...
          // params: sig num pubkey
          // retruns: num
          if (stack.length < 3)
            throw new ScriptError('INVALID_STACK_OPERATION', op, ip);

          const sig = stack.get(-3);
          const num = stack.getNum(-2, minimal, 4);
          const key = stack.get(-1);
          ...
          const res = this.evalChecksigTapscript(
            tx,
            key,
            sig,
            sighashData,
          ...
          stack.pop();
          stack.pop();
          stack.pushNum(res ? num.add(new ScriptNum(1)) : num);
  ...
  /**
   * Test whether the output script is
   * pay-to-taproot (witness program v1)
   * @returns {Boolean}
   */
  ...
  static verify(tx, index, coins, flags) {
    const input = tx.inputs[index].script;
    const witness = tx.inputs[index].witness;
    const coin = coins[index];
    const output = coin.script;
    ...
    input.execute(stack, flags, tx, index, value, digests.LEGACY);
    ...
    output.execute(stack, flags, tx, index, value, digests.LEGACY);
    ...
    // Native segregated witness (P2WPKH, P2WSH, P2TR)
    if ((flags & Script.flags.VERIFY_WITNESS) && output.isProgram()) {
      ...
      Script.verifyProgram(witness, output, flags, tx, index, coins, false);
    ...
    if ((flags & Script.flags.VERIFY_P2SH) && output.isScripthash()) {
      ...
      redeem.execute(stack, flags, tx, index, value, digests.LEGACY);
      ...
      if ((flags & Script.flags.VERIFY_WITNESS) && redeem.isProgram()) {
        ...
        Script.verifyProgram(witness, redeem, flags, tx, index, coins, true);
   ...
   * @param {Number} index
   * @param {Coin[]} coins
   * @param {Boolean} p2sh - whether the output is nested in P2SH
   * @throws {ScriptError}
   */

  static verifyProgram(witness, output, flags, tx, index, coins, p2sh = false) {
    const coin = coins[index];
    const value = coin.value;
    ...
    if (program.version === 0) {
    ...
    } else if (program.version === 1 && program.data.length === 32 && !p2sh) {
      // Soft-fork logic: Enforce Taproot rules or pass without error.
      if (!(flags & Script.flags.VERIFY_TAPROOT))
        return;
      ...
      // Annex use is currently undefined and discouraged (non-standard).
      // Otherwise, remove the annex from the stack if present.
      if (witness.getAnnex()) {
        // Annex is always the last item in the stack.
        stack.pop();
      }
      ...
      if (stack.length === 1) {
        ...
        // In Taproot, SIGHASH_ALL is default (64 byte sig).
        // A 65th byte signals an alternate sighash (specifying ALL is invalid).
        // All other sig lengths are invalid.
        const sig = stack.pop();
        const type = getTaprootSighashType(sig);
        ...
        if (checksigSchnorr(hash, sig.slice(0, 64), program.data))
          return;
        else
          throw new ScriptError('TAPROOT_INVALID_SIG');
      } else {
        // Script path spending

        const controlBlock = stack.pop();
        const script = stack.pop();

        // Check control block size
        const size = controlBlock.length;
        if (size < 33
            || size > 33 + (32 * 128)
            || (size - 33) % 32 !== 0) {
        ...
        // Verify the script is committed to by the output address
        if (!Script.verifyTaprootCommitment(witness, output))
          throw new ScriptError('WITNESS_PROGRAM_MISMATCH');

        // Check the TapLeaf version
        const leafVersion = controlBlock[0] & 0xfe;

        // Only one version is currently defined: "TapScript"
        if (leafVersion === common.LEAF_VERSION_TAPSCRIPT) {
          digest = digests.TAPSCRIPT;
          redeem = Script.fromRaw(script);
          // Fill up the gas tank
          redeem._sigopBudget =
            consensus.VALIDATION_WEIGHT_OFFSET + witness.getVarSize();

          // "OP_SUCCESSx processing overrides everything,
          // including stack element size limits"
          // Bitcoin Core src/script/interpreter.cpp ExecuteWitnessScript()
          for (let ip = 0; ip < redeem.code.length; ip++) {
            const op = redeem.code[ip];
            if (op.value === -1)
              throw new ScriptError('BAD_OPCODE', op, ip);

            if (op.isSuccess()) {
              if (flags & Script.flags.VERIFY_DISCOURAGE_OP_SUCCESS)
          ...
          if (flags & Script.flags.VERIFY_DISCOURAGE_UPGRADABLE_TAPROOT_VERSION)
    ...
    assert(digest != null);
    redeem.execute(stack, flags, tx, index, value, digest, coins);
  ...
  static verifyTaprootCommitment(witness, output) {
    const control = witness.getControlBlock();
    const script = witness.getTapleaf();
    const program = output.getProgram();

    assert(program.version === 1,
      'verifyTaprootCommitment called on non-taproot witness program version.');
    assert(control, 'Missing control block for Taproot script path spend.');
    assert(script, 'Missing Tapleaf script for Taproot script path spend.');

    // Start reading from the Control Block
    const bControl = bio.read(control);
    const controlByte = bControl.readU8();
    const leafVersion = controlByte & 0xfe;
    const sign = (controlByte & 1) === 1;
    const internalKey = bControl.readBytes(32);

    // Valid output key required.
    const outputKey = program.data;
    if(!schnorr.publicKeyVerify(outputKey))
      return false;

    // Compute the Tapleaf hash (leaf version + script)
    const leafSize = encoding.sizeVarBytes(script);
    const tapLeaf = bio.write(leafSize + 1);
    tapLeaf.writeU8(leafVersion);
    tapLeaf.writeVarBytes(script);
    let k = taggedHash.TapLeafHash.digest(tapLeaf.render());

    // Compute merkleized script tree root, starting with the Tapleaf hash
    while (bControl.left() > 0) {
      const next = bControl.readBytes(32);
      const tapBranch = bio.write(64);
      if (strcmp(k, next) < 0) {
        tapBranch.writeBytes(k);
        tapBranch.writeBytes(next);
      } else {
        tapBranch.writeBytes(next);
        tapBranch.writeBytes(k);
      }
      k = taggedHash.TapBranchHash.digest(tapBranch.render());
    }

    // Tweak the interal key with the script tree root
    const tapTweak = bio.write(64);
    tapTweak.writeBytes(internalKey);
    tapTweak.writeBytes(k);
    const t = taggedHash.TapTweakHash.digest(tapTweak.render());

    // Test tweaked key against the Taproot output key
    return schnorr.publicKeyTweakCheck(internalKey, t, outputKey, sign);
  }

/**
 * Verify Tapscript signature rules including soft fork exemptions.
 * @param {TX} tx
 * @param {Buffer} key
 * @param {Buffer} sig
 * @param {Object} sighashData
 * @param {Number} flags
 * @returns {Boolean}
 */

evalChecksigTapscript(tx, key, sig, sighashData, flags) {
  // Empty sig returns false but doesn't necessarily throw an error.
  // OP_CHECKSIG will push a false on the stack and continue.
  const success = sig.length !== 0;

  if (success) {
    // Pay for this sigop and check the remaining budget in the gas tank
    assert(this._sigopBudget != null);
    this._sigopBudget -= consensus.VALIDATION_WEIGHT_PER_SIGOP_PASSED;
    if (this._sigopBudget < 0)
  ...
  if (key.length === 0) {
    ...
    // Return false BEFORE checking the signature.
    // If we call getTaprootSighashType() or checksigSchnorr()
    // with an empty signature, they'll throw an error and stop
    // script execution. From BIP342: "If the signature is the
    // empty vector... an empty vector is pushed onto the stack,
    // and execution continues with the next opcode.
    if (!success)
      return false;

    // Everything is finally approved for Tapscript
    // sighash computation and signature validation.
    const type = getTaprootSighashType(sig);
    const hash = tx.signatureHash(
      sighashData.index,
      null, // signatureHashTaproot() does not require prev script / subscript
      sighashData.value,
      type,
      sighashData.digest, // digests.TAPSCRIPT
      sighashData.coins,
      sighashData.codeSepPos);
    if (!checksigSchnorr(hash, sig.slice(0, 64), key))
      throw new ScriptError('NULLFAIL');
  } else {
    // Unknown pubkey type is upgradeable via soft fork
    if (flags & Script.flags.VERIFY_DISCOURAGE_UPGRADABLE_PUBKEYTYPE)
...
Script.digestsByVal = common.digestsByVal;
...
Script.isTaprootHashType = common.isTaprootHashType;
 ...
 * @param {Buffer} key
 * @param {VerifyFlags?} flags
 * @param {Number} digest type
 * @returns {Boolean}
 * @throws {ScriptError}
 */

function validateKey(key, flags, digest) {
  assert(Buffer.isBuffer(key));
  assert(typeof flags === 'number');
  assert(typeof digest === 'number');
...
/**
 * Verify a schnorr signature, sighash type byte should already be
...
function checksigSchnorr(msg, sig, key) {
  return schnorr.verify(msg, sig, key);
}

/**
 * Get the sighash type from a Taproot signature
 * @param {Buffer} sig
 * @returns {number}
 */

function getTaprootSighashType(sig) {
  let type = 0;
  if (sig.length === 65) {
